...
- **Performance optimisations**
  ...
  - **Single `craft_relations` query for outgoing relationships** — `findOutgoingRelationships()` now issues a single `SELECT fieldId, targetId FROM craft_relations WHERE sourceId = :id`, builds a `targetId → [fieldIds]` map, then loads elements per-type via `->id($allTargetIds)` queries — matching the approach already used by `findIncomingRelationships()`. Field handles for the tooltip chips are resolved via `getFieldById()`, which Craft caches internally.
